...
#SBATCH -o /scratch/emart459/PSB6351/crash/preproc_o
#SBATCH -e /scratch/emart459/PSB6351/crash/preproc_e
...
base_dir = '/home/emart459/psb6351_github/mattfeld_2020'
work_dir = '/scratch/emart459/PSB6351'
...
# I accidentally somehow deleted some of the script so here is me typing it out again
...
#Calculate transformation matrix with BBRegister - FreeSurfer
...
# Spatial smoothing using AFNI's BlurToFWHM function
blur = pe.MapNode(afni.BlurToFWHM(),
			iterfield=['in_file'],
			name = 'blur')
#blur.inputs.in_file = func_files #gets input files from here --> met with group who told me not to do this?
blur.inputs.fwhm = 6.0 #which mm for this dataset? testing 3,6,9,12mm
blur.inputs.outputtype = 'NIFTI_GZ' #I want output file to be in NIFTI_GZ format
blur.inputs.automask = True #testing arguments for BlurToFWHM
psb6351_wf.connect(tshifter, 'out_file', blur, 'in_file')

#Temporal smoothing using AFNI's tSmooth
tsmooth = pe.MapNode(afni.TSmooth(),
		     iterfield=['in_file'],
		     name = 'tSmooth')
tsmooth.inputs.outputtype = 'NIFTI_GZ'
tsmooth.inputs.lin = True #default
tsmooth.inputs.adaptive = 5 #standard value for this number from nipype: can use any odd integer starting with 3
psb6351_wf.connect(blur, 'out_file', tsmooth, 'in_file')
...
psb6351_wf.connect(blur, 'out_file', datasink, 'spatial_smooth') #added spatial smoothing output folder
psb6351_wf.connect(tsmooth, 'out_file', datasink, 'temp_smooth') #added temporal smoothing output folder
